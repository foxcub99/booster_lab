...
class T1Env(DirectRLEnv):
    ...
    def __init__(self, cfg: T1EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Command tracking
        self.max_episode_length = int(self.cfg.episode_length_s / self.step_dt)
    ...
    def _get_joint_indices(self):
        ...
        self.leg_dof_indices = torch.tensor(self.leg_dof_indices, dtype=torch.long, device=self.device)
        ...
        self.contact_body_indices = torch.tensor(self.contact_body_indices, dtype=torch.long, device=self.device)
    ...
    def _init_command_terms(self):
        ...
        self.command_sums = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
    ...
    def _setup_scene(self):
        ...
        # Setup joint indices after scene is created
    ...
    def _apply_action(self) -> None:
        ...
        if self.leg_dof_indices is not None and len(self.leg_dof_indices) > 0:
            self.robot.set_joint_position_target(self.actions, joint_ids=self.leg_dof_indices)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Torque penalty
        torque_penalty = torch.sum(torch.square(self.robot.data.applied_torque), dim=1) * self.cfg.torque_reward_scale

        # Joint acceleration penalty  
        joint_acc = (self.robot.data.joint_vel[:, self.leg_dof_indices] - self.last_joint_vel) / self.step_dt
        joint_acc_penalty = torch.sum(torch.square(joint_acc), dim=1) * self.cfg.joint_accel_reward_scale

        # Action rate penalty
        action_rate_penalty = torch.sum(torch.square(self.actions - self.last_actions), dim=1) * self.cfg.action_rate_reward_scale

        # Orientation penalty (keep robot upright)
        base_quat = self.robot.data.root_quat_w
        up_proj = 2 * (base_quat[:, 1] * base_quat[:, 3] + base_quat[:, 0] * base_quat[:, 2])
        orientation_penalty = torch.square(up_proj) * self.cfg.flat_orientation_reward_scale
        ...
        self.last_joint_vel = self.robot.data.joint_vel[:, self.leg_dof_indices].clone()
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        roll = torch.atan2(2 * (base_quat[:, 0] * base_quat[:, 1] + base_quat[:, 2] * base_quat[:, 3]),
                          1 - 2 * (base_quat[:, 1]**2 + base_quat[:, 2]**2))
        pitch = torch.asin(2 * (base_quat[:, 0] * base_quat[:, 2] - base_quat[:, 3] * base_quat[:, 1]))
        
        fallen = (base_height < 0.3) | (torch.abs(roll) > 1.0) | (torch.abs(pitch) > 1.0)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        root_pos[:, 0] += torch.uniform(-0.5, 0.5, (len(env_ids),), device=self.device)
        root_pos[:, 1] += torch.uniform(-0.5, 0.5, (len(env_ids),), device=self.device)
        root_pos[:, 2] += torch.uniform(-0.1, 0.1, (len(env_ids),), device=self.device)

        # Randomize initial orientation slightly
        root_quat = self.robot.data.default_root_state[env_ids, 3:7]
        root_quat[:, 3] += torch.uniform(-0.1, 0.1, (len(env_ids),), device=self.device)
        root_quat = torch.nn.functional.normalize(root_quat, dim=1)

        # Randomize joint positions slightly around default
        joint_pos = self.robot.data.default_joint_pos[env_ids]
        joint_pos += torch.uniform(-0.1, 0.1, joint_pos.shape, device=self.device)
        ...
        # Apply the reset
        self.robot.write_root_pose_to_sim(root_pos, root_quat, env_ids)
        self.robot.write_root_velocity_to_sim(root_vel, env_ids)
        self.robot.write_joint_state_to_sim(joint_pos, joint_vel, None, env_ids)

        # Reset tracking variables
        if not hasattr(self, 'last_joint_vel'):
            self.last_joint_vel = torch.zeros((self.num_envs, len(self.leg_dof_indices)), device=self.device)
    ...
    def _resample_commands(self, env_ids: torch.Tensor):
        ...
        self.commands[env_ids, 0] = torch.uniform(
            -self.cfg.max_command_speed, self.cfg.max_command_speed, 
        ...
        self.commands[env_ids, 2] = torch.uniform(
            -self.cfg.max_command_angular_speed, self.cfg.max_command_angular_speed,
            (len(env_ids),), device=self.device
        )
